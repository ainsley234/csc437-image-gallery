...
export function registerImageRoutes(app, imageProvider) {
    ...
    app.put("/api/image/:id/name", async (req, res) => {
        ...
        try {
            ...
            if (req.userInfo.username != img.authorId) { throw new Error("403") } //authorId is the username
            ...
            if (err.message == "413") {
                return res.status(413).send({
                    ...
                    message: `Image name exceeds ${getEnvVar("MAX_NAME_LENGTH")} characters`
                });
            }
            console.log(err)
    ...
    app.post(
        ...
        async (req, res) => {
            console.log("I got to post /api/images")
            ...
            console.log(getEnvVar("MAX_NAME_LENGTH"))
            if (req.body.name.length > getEnvVar("MAX_NAME_LENGTH")) {return res.status(413).send("Image title is too long.");}

            console.log("username is:", req.userInfo.username)
